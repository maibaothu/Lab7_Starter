...
function initializeServiceWorker() {
  ...
  //            log that it has failed.
      console.error("Registeration Fails!");
      }
    });
  }
...
async function getRecipes() {
  ...
  //            declared above 
    for (let recipeIndex = 0; recipeIndex < RECIPE_URLS.length; recipeIndex++) {
  ...
  //            function is using the async keyword so we recommend "await"
        const fetchRecipe = await fetch(RECIPE_URLS[recipeIndex]);
  ...
  //            "await" again
        const fetchJSON = await fetchRecipe.json();
  // A8. TODO - Add the new recipe to the recipes array
        recipeArray[recipeIndex] = fetchJSON;
  ...
  //            resolve() method.
        if (recipeIndex + 1 == RECIPE_URLS.length) {
